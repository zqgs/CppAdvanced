...
//问题不在基类Cl34_Airplane::fly的缺省行为,而在于Model C未能提供C的重写版本,导致调用了基类缺省版本。
...
//函数代表的意义是不变性凌驾特异性，所以它绝不该在派生类中重新定义。
...
//常犯的错误(但不绝对):
//1.将所有成员函数都声明为non-virtual函数,这使得派生类没有剩余空间进行特化工作。如果并不想设计一个基类的话，那non-virtual就是合理的。
